...
class BitcoinViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if let eur = UserDefaults.standard.string(forKey: "INR") {
            eurLabel.text = eur
        }
    ...
    func getPrice() {
        if let url = URL(string: "https://min-api.cryptocompare.com/data/price?fsym=BTC&tsyms=USD,JPY,INR") {
                    ...
                    if data != nil {
                        if let json = try? JSONSerialization.jsonObject(with: data!, options: []) as? [String:Double] {
                            DispatchQueue.main.async {
                                ...
                                if let eurPrice = json["INR"] {
                                    self.eurLabel.text = self.getStringFor(price: eurPrice, currencyCode: "INR")
                                    UserDefaults.standard.set(self.getStringFor(price: eurPrice, currencyCode: "INR") + "~", forKey: "INR")
                                }
